...
{
    public class GraphNodeResolver<TSource, TKey, TReturn> : IFieldResolver
    {
        ...
        public GraphNodeResolver(Func<TSource, TKey> keySelector, Func<ResolveFieldContext<IEnumerable<TKey>>, Task<ILookup<TKey, TReturn>>> resolver)
        {
        ...
        public async Task<object> ResolveInternal(ResolveFieldContext context)
        {
            ...
            var result = data[_keySelector((TSource)node.Value)];
            return new GraphNodeCollection<TReturn>(result);
        }
